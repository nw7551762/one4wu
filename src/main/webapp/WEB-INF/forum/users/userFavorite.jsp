...
<!-- Title  -->
<title>UniversalHouse | 我的最愛</title>
...
<body>
		...
		<header class="header-area clearfix">
			...
			<div class="cart-fav-search mb-100">
				<a href="/users/showMemberInfo" class="cart-nav"><img
					...
					href="#" class="fav-nav"><img src="/amado-master/img/core-img/favorites.png" alt="">我的最愛</a> <a
							...
							<div class="row" style="margin-left:100px">
								<form method="post" action="#">
									...
									<ul class="topicName">
										...
										<li>&nbsp;|</li>
										<li><input type="submit" style="background-color: transparent;border: none;text-align: inherit;color:red;font-size: inherit;cursor: pointer;transition: color 0.5s;" formaction="/users/toUserFavorite" value="我的最愛"/></li>
										<li>&nbsp;|</li>
				...
				<div class="row">
					<div class="col-12">
						<div class="checkout_details_area mt-50 clearfix">
							...
							<div class="row">
								<div class="col-12 mb-3" id="myFavorite">
									<hr>
									<c:choose>
									   <c:when test="${empty myFavorite}">
									    	<h3>您還未新增文章到我的最愛!</h3>
									    </c:when>
								   	 	<c:otherwise>
											<c:forEach var="article" varStatus="stat" items="${myFavorite}">
												...
												<div id="${stat.index}" style="background-color: #F5F5DC;height: 150px; margin-bottom: 20px;border-radius: 5px">
													...
													<div style="float:right;margin-right:30px;margin-top:80px">
														<form id="modify" action="#">
															...
															<input type="button" class="btn" id="cancel" style="background-color:#FF7575" value="取消收藏" />
														</form>
													</div>
												</div>
											</c:forEach>
	...
	$.ajax({
		...
		success : function(member) {
			...
			if (member.length != 0) {
				...
				$('#welcomeTitle').html(member.name + ' 的最愛');
				currentUser = member;
	...
	$('#myFavorite').on('click','#cancel',function() {
	  ...
	  .then((willDelete) => {
	    if (willDelete) {
	      $.ajax({
	        ...
	        success : function() {
	          swal("已從我的最愛中移除", "", "success");
	          thisDeleteButton.parent().parent().parent().detach();
	        },
	        
	        error : function(error) {
	          swal("錯誤", "updateStatus : 失敗", "error");
	        },
	      });
	    } else {
	      swal("取消操作");
